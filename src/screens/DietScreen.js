...
function IconCircle({ name, tone = 'brand', size = 20 }) {
  ...
  return <View style={[styles.iconCircle, sage && styles.iconCircleSage]}><Icon name={name} size={size} color={sage ? COLORS.sage : COLORS.brand} /></View>;
}
...
function SectionHeader({ title, subtitle, action, onAction }) {
  return (
    <View style={styles.sectionHeader}>
      ...
      {action ? <Pressable onPress={onAction} accessibilityRole='button' style={interactive(styles.textAction)}><Text style={styles.textActionLabel}>{action}</Text><Icon name='arrow-forward' size={16} color={COLORS.brand} /></Pressable> : null}
    </View>
  );
}
...
export default function DietScreen({ navigation, route }) {
  ...
  if (state.isLoading) return <SafeAreaView style={styles.safeArea}><View style={styles.loading}><Icon name='nutrition-outline' size={28} color={COLORS.brand} /><Text style={styles.loadingText}>Preparing Diet…</Text></View></SafeAreaView>;
